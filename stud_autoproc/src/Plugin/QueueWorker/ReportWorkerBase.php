...
abstract class ReportWorkerBase extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  protected function reportWork($worker, $item) {
    if ($this->state->get('stud_autoproc_show_status_message')) {
    ...
    $this->logger->get('stud_autoproc')->info('Queue @worker worker processed item with sequence @sequence created at @time', [
      '@worker' => $worker,
      '@sequence' => $item->sequence,
      '@time' => date('c', $item->created),
    ]);
